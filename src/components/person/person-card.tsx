...
export function PersonCard({ person }: PersonCardProps) {
  return (
    <Link href={`/people/${person.mal_id}`} className="group block">
      <Card className="h-full overflow-hidden transition-all duration-300 group-hover:shadow-lg group-hover:shadow-primary/20 group-hover:border-primary/50 group-hover:-translate-y-1">
        <CardHeader className="p-0">
          <div className="aspect-[2/3] relative">
            <Image
              src={person.images.jpg.image_url || 'https://placehold.co/200x300.png'}
        ...
        <CardContent className="p-2 text-center">
            <p className="text-sm font-semibold truncate font-headline" title={person.name}>
                {person.name}
            </p>
        </CardContent>
